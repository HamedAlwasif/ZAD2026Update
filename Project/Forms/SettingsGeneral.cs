    ...
    public partial class SettingsGeneral : Form
    {
        ...
        private void butUbdateBill_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // string connectionString = "Data Source=.\\SQLEXPRESS;Initial Catalog=Almenna;Integrated Security=True";

                using (SqlConnection conn = new SqlConnection(constring))
                {
                    string query = @"UPDATE SystemProgram 
                         SET PriceKataeeProfitRate=@Kataee, 
                             PriceAlgomlaProfitRate=@Algomla, 
                             PriceNesfAlgomlaProfitRate=@NesfAlgomla, 
                             PriceGomlaAlgomlaProfitRate=@GomlaAlgomla";

                    SqlCommand cmd = new SqlCommand(query, conn);
                    cmd.Parameters.AddWithValue("@Kataee", textPriceKataeeProfitRate.Text);
                    cmd.Parameters.AddWithValue("@Algomla", textPriceAlgomlaProfitRate.Text);
                    cmd.Parameters.AddWithValue("@NesfAlgomla", textPriceNesfAlgomlaProfitRate.Text);
                    cmd.Parameters.AddWithValue("@GomlaAlgomla", textPriceGomlaAlgomlaProfitRate.Text);

                    conn.Open();
                    int rowsAffected = cmd.ExecuteNonQuery();

                   // if (rowsAffected > 0)
                   //  MessageBox.Show("✅ تم حفظ النسب بنجاح");
                   //else
                   //MessageBox.Show("⚠️ لم يتم التحديث (تأكد أن جدول SystemProgram يحتوي على بيانات)");
            }
        ...
        private void butUbdateBarcode_Click(object sender, EventArgs e)
        {
           ...
           sqlCommand1.Parameters.Add("@BarcodeSeparator", SqlDbType.Float).Value = textBarcodeSeparator.Text;
            sqlCommand1.ExecuteNonQuery();
            sqlCommand1.Parameters.Clear();
            ...
            AppSetting.BarcodeSeparator = textBarcodeSeparator.Text;
        ...
        private void butProfitPercentDefult_Click(object sender, EventArgs e)
        {
