    ...
    public partial class Prices : Form
    {
        ...
        private void Prices_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                combCategorys2.DataSource = Dt1;
                combCategorys2.DisplayMember = "Category";
                combCategorysSerch.DataSource = Dt1;
                combCategorysSerch.DisplayMember = "Category";
            }
            catch
            {
            ...
            //------------- تلوين الصفوف التى تم تعديل البينات بها

            dataGridViewCategory.CellValueChanged += dataGridViewCategory_CellValueChanged;
            dataGridViewCategory.CurrentCellDirtyStateChanged += dataGridViewCategory_CurrentCellDirtyStateChanged;

            //--------- ربط حدث دالة منع النصوص فى الجدول
            dataGridViewCategory.EditingControlShowing += dataGridViewCategory_EditingControlShowing;

            dataGridViewCategory.DataError += dataGridViewCategory_DataError;

            //----------------------
            combCategorysSerch.TextChanged += combCategorysSerch_TextChanged;
        }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlDataAdapter da11 = new SqlDataAdapter("select * from Category ", cn);
                da11.Fill(dt11);
                this.dataGridViewCategory.DataSource = dt11;

            }
            catch
            { }

            textcountCategory.Text = (dataGridViewCategory.RowCount - 1).ToString(); // عدد الاصناف
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                "select * from Category where Category = @Category", cn);

                da11.SelectCommand.Parameters.AddWithValue("@Category", combCategorys2.Text);

                dtCategory.Clear();
                da11.Fill(dtCategory);
                ...
                //DataTable dt11 = new DataTable();
                //dt11.Clear();
                //SqlDataAdapter da11 = new SqlDataAdapter("select * from Category where  Category ='" + combCategorys2.Text + "'  ", cn);
                //da11.Fill(dt11);
                //this.dataGridViewCategory.DataSource = dt11;
        ...
        private void SearchByBarcode(string Barcode)
        {
            ...
            SqlDataAdapter da11 = new SqlDataAdapter("select * from Category where Barcode = @Barcode", cn);

            da11.SelectCommand.Parameters.AddWithValue("@Barcode", Barcode);
        ...
        private void dataGridViewCategory_CurrentCellDirtyStateChanged(object sender, EventArgs e)
        {
            if (dataGridViewCategory.IsCurrentCellDirty)
            {
                dataGridViewCategory.CommitEdit(DataGridViewDataErrorContexts.Commit);
            }
        }

        private void butSave_Click(object sender, EventArgs e)
        {
            //====== حفظ الصفوف المعدلة فقط

            dataGridViewCategory.EndEdit();
        ...
        UPDATE Category SET
            Faction = @Faction,
            Price = @Price,
            PriceGomla = @PriceGomla,
            PriceMostahlek = @PriceMostahlek,
            PriceGomlaAlgomla = @PriceGomlaAlgomla,
            PriceNesfAlgomla = @PriceNesfAlgomla
        WHERE Barcode = @Barcode", cn);

                cmd.Parameters.Add("@Barcode", SqlDbType.NVarChar, 10);
                cmd.Parameters.Add("@Faction", SqlDbType.NVarChar, 50);
                cmd.Parameters.Add("@Price", SqlDbType.Float);
                cmd.Parameters.Add("@PriceGomla", SqlDbType.Float);
                cmd.Parameters.Add("@PriceMostahlek", SqlDbType.Float);
                cmd.Parameters.Add("@PriceGomlaAlgomla", SqlDbType.Float);
                cmd.Parameters.Add("@PriceNesfAlgomla", SqlDbType.Float);
                ...
                foreach (DataGridViewRow row in dataGridViewCategory.Rows)
                {
                    ...
                    // حفظ الصفوف المعدلة فقط (اللون الأصفر)
                    ...
                    cmd.Parameters["@Barcode"].Value = row.Cells["Barcode"].Value.ToString();

                    cmd.Parameters["@Faction"].Value =
                        row.Cells["Faction"].Value ?? (object)DBNull.Value;

                    cmd.Parameters["@Price"].Value =
                        row.Cells["Price"].Value ?? (object)DBNull.Value;

                    cmd.Parameters["@PriceGomla"].Value =
                        row.Cells["PriceGomla"].Value ?? (object)DBNull.Value;

                    cmd.Parameters["@PriceMostahlek"].Value =
                        row.Cells["PriceMostahlek"].Value ?? (object)DBNull.Value;

                    cmd.Parameters["@PriceGomlaAlgomla"].Value =
                        row.Cells["PriceGomlaAlgomlaColumn"].Value ?? (object)DBNull.Value;

                    cmd.Parameters["@PriceNesfAlgomla"].Value =
                        row.Cells["PriceNesfAlgomlaColumn"].Value ?? (object)DBNull.Value;
                ...
                MessageBox.Show("تم حفظ " + counter + " صنف فقط ✅");
            }
            catch (Exception ex)
            {
            ...
            //        cn.Close();
            //}
            //- رجوع لون الصفوف للاصل
            foreach (DataGridViewRow row in dataGridViewCategory.Rows)
            {
                //- اللون الاصلى الابيض
                row.DefaultCellStyle.BackColor = Color.White;

                //--------- لو عاوز اللون يبقى اخضر ليدل على الحفظ
               // row.DefaultCellStyle.BackColor = Color.LightGreen;
            }
        }
        ...
        private void butCancel_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("تم إلغاء جميع التعديلات ❌");
        }
        ...
        //--------- منع كتابة نصوص فى خانات الاسعار فى الجدول
        private void dataGridViewCategory_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
                    if (dataGridViewCategory.CurrentCell.ColumnIndex == dataGridViewCategory.Columns["Price"].Index ||
               dataGridViewCategory.CurrentCell.ColumnIndex == dataGridViewCategory.Columns["PriceGomla"].Index ||
               dataGridViewCategory.CurrentCell.ColumnIndex == dataGridViewCategory.Columns["PriceMostahlek"].Index ||
               dataGridViewCategory.CurrentCell.ColumnIndex == dataGridViewCategory.Columns["PriceGomlaAlgomlaColumn"].Index ||
               dataGridViewCategory.CurrentCell.ColumnIndex == dataGridViewCategory.Columns["PriceNesfAlgomlaColumn"].Index)
                    {
        ...
        {
            MessageBox.Show("مسموح بالأرقام فقط فى خانات الأسعار");
            e.Cancel = true;
        }

        // === حدث قبل تعديل البيانات فى الجدول للرجوع لها عند الالغاء
        private void dataGridViewCategory_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            var cell = dataGridViewCategory.Rows[e.RowIndex].Cells[e.ColumnIndex];

            if (cell.Tag == null)
                cell.Tag = cell.Value;
        }

        private void combCategorysSerch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                if (combCategorysSerch.Text == "")
                {
                    query = "SELECT * FROM Category";
                }
                else
                {
                    query = "SELECT * FROM Category WHERE Category LIKE @Search";
                }
