...
{
    public partial class MoneyToBox : Form
    {
        //----------------- ConnectionStrings ------------------
        ...
        public void GetDataTable()
        {
            sqlConnection1.Open();
                ...
                DataTable dt11 = new DataTable();
                dt11.Clear();
                SqlDataAdapter da11 = new SqlDataAdapter("select ID as م ,Date as التاريخ,Name as الاسم,Report as البيان,Wared as الوارد from Movemoney where Wared >'" + 0 + "' ", sqlConnection1);
                da11.Fill(dt11);
                this.dataGridView1.DataSource = dt11;
                // ***  إجمالى المصاريف  ****
                try
        ...
        private void MoneyToBox_Load(object sender, EventArgs e)
        {
            ...
            if (TypeMoneyToBox == "إضافة نقدية للصندوق أول المدة")
            {
                butAddFristTime.Visible = true;
                butAdd.Visible = false;

                GetFristGardID();

            }
            else if (TypeMoneyToBox == "إضافة نقدية للصندوق")
            {
                butAddFristTime.Visible = false;
                butAdd.Visible = true;
            }
        ...
        private void butAddFristTime_Click(object sender, EventArgs e)
        {
            DialogResult dialogResult = MessageBox.Show("نص الرسالة ." + Environment.NewLine + Environment.NewLine + "هل تريد إضافة نقدية بالفعل ؟", "إستفسار", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (dialogResult == DialogResult.Yes)
            {
                sqlConnection1.Open();
                try
                {
                    sqlCommand1.CommandText = "insert into FristGard (ID,Date,Move,Name,GardFrist,Madenon,Daenon,Box,Building,Electronic,BasisOFFICE,Bank,adv)values ('" + FristGardID + "','" + dateTimePicker1.Value.ToString("MM/dd/yyyy") + "','" + TypeMoneyToBox + "','" + texUser.Text + "','" + 0 + "','" + 0 + "','" + 0 + "','" + textBox1.Text + "','" + 0 + "','" + 0 + "','" + 0 + "','" + 0 + "','" + 0 + "')";
                ...
                try
                {
                    sqlCommand1.CommandText = "insert into BoxMove (ID,Date,Move,Name,NumBill,Remaining,Sader,Wared,Total,Note)values ('" + MoveBoxID + "','" + dateTimePicker1.Value.ToString("MM/dd/yyyy") + "','" + TypeMoneyToBox + "','" + texUser.Text + "','" + s + "','" + txtReminngOLD.Text + "','" + 0 + "','" + textBox1.Text + "','" + w1S + "','" + textBox3.Text + "')";
                    sqlCommand1.ExecuteNonQuery();
        ...
        private void butAdd_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                sqlCommand1.CommandText = "insert into Movemoney (Date,Name,Report,Sader,Wared)values ('" + dateTimePicker1.Value.ToString("MM/dd/yyyy") + "','" + texUser.Text + "','" + textBox3.Text + "','" + 0 + "','" + textBox1.Text + "')";
                sqlCommand1.ExecuteNonQuery();
